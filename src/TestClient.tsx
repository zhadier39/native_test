...
const TestClient = () => {
  ...
  const [accounts] = useState<string[]>([
    'Hyperengage', 'Acme', 'Globex', 'Initech', 'Umbrella', 'Stark', 'Wayne', 'Hooli', 'Aperture', 'Vehement',
    'Wonka', 'Massive Dynamic', 'Soylent', 'Cyberdyne', 'Pied Piper', 'Oscorp', 'Duff', 'Dunder Mifflin', 'Gringotts', 'Tyrell'
  ]);
  ...
  const countries = [
    'United States', 'Canada', 'United Kingdom', 'Germany', 'France', 'India', 'Australia', 'Brazil', 'Singapore', 'Netherlands'
  ];
  const planTiers = ['free', 'starter', 'growth', 'scale', 'enterprise'];
  const lifecycleStages = ['lead', 'trial', 'customer', 'expansion', 'churn_risk'];
  ...
  const eventCatalog = useMemo(
    () => [
      ...
      // Signals
      'signal_created', 'signal_acknowledged', 'signal_resolved', 'churn_warning_created', 'upsell_opportunity_created',
  ...
  const makeAccount = (idx: number): Account => {
    const baseName = accounts[idx % accounts.length];
    const suffix = idx.toString().padStart(4, '0');
    const name = `${baseName} ${suffix}`;
    const domain = `${slugify(baseName)}${suffix}.com`;
    ...
    return {
      ...
      traits: {
        ...
        lifecycle_stage: pick(lifecycleStages),
        signup_date: createdAt.toISOString(),
        health_score: Math.round(randomFloat(20, 100)),
        churn_risk_score: Math.round(randomFloat(1, 100)),
      },
    };
  };

  const randomName = () => {
    const first = ['Ava', 'Liam', 'Noah', 'Emma', 'Olivia', 'Sophia', 'Mia', 'Lucas', 'Ethan', 'Amelia', 'Isabella', 'James', 'Benjamin', 'Elijah'];
    const last = ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis', 'Rodriguez', 'Martinez', 'Hernandez', 'Lopez'];
  ...
  const makeUser = (account: Account, idx: number): User => {
    ...
    const tz = pick(['UTC', 'PST', 'EST', 'CET', 'IST', 'AEST']);
